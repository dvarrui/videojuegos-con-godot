...
Vamos a crear el efecto **white flash** usando shaders.
...
* Para probar lo que hemos creado, vamos a `ShaderMaterial` y en el inspector modificamos el valor de las siguientes propiedades para ver el efecto que produce:
...
En el apartado anterior vimos como hacer un shader que se puede usar para producir el efecto **white flash**. Pero para darle uso, en cada sprite del juego habría que hacer lo siguiente:
...
* Dejar pasar un tiempo y volver a poner el valor de la propiedad `flash_modifier` a 0.0.
